...
function graphene (f, s = 100, top = process.argv.includes('-top') ? process.stdout.rows - 3 : Infinity) {

	if (cluster.isMaster) { console.log('starting')
		...
		function visualize () { console.log('printing')
			...
			for (let e of echo) {
				...
				let x = (visualm + e[1]).slice(- String(Math.max(...echo.values())).length)
				if (Number(x) === 0) x = ''
				console.info((visualm + e[0]).slice(- String(Math.max(...echo.keys())).length), (visual + visualm).slice(0, visualm.length), x)
			}
			console.info('')
